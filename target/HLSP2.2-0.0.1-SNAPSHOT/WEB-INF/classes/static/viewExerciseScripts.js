...
function processAerobicTableResponse(data) {
      ...
      data.forEach(function(i){
			...
			(	
				...
				+ '<td>' + i.steps + '</td>'
				+ '<td>' + i.favourite + '</td>'
				+ '<td><a class="btn btn-secondary btn-sm" href="/HLSP/exercise/edit_aerobic_exercise?id=' + i.id + '" role="button">Edit</a></td>'
			 + "</tr>"				
			);
		});				 	
}
...
function processStrengthTableResponse(data) {
      ...
      data.forEach(function(i){
			...
			(	
				...
				+ "<td>" + i.reps + "</td>"
				+ "<td>" + i.favourite + "</td>"
				+ '<td><a class="btn btn-secondary btn-sm" href="/HLSP/exercise/edit_strength_exercise?id=' + i.id + '" role="button">Edit</a></td>'
			 + "</tr>"				
			);
		});				 	
}
